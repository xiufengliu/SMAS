...
DROP TABLE smas_power_consumptionpattern;
CREATE TABLE smas_power_consumptionpattern(
    meterid integer primary key,
    tenpct float8[],
    tenpct_C integer,
    fiftypct float8[],
...
DROP TABLE smas_power_consumptionpattern_centroid;
CREATE TABLE smas_power_consumptionpattern_centroid(
    measure varchar(30),
    clusterid integer,
    cent float8[]
);
...
INSERT INTO smas_power_consumptionpattern(
 ...
 FROM (
	 SELECT
		meterid,
		extract(hour from readtime),
		count(*) AS cnt,
		array_agg(reading ORDER BY reading) AS readarr,
		avg(reading) AS AvgReading
	 FROM smas_power_hourlyreading GROUP BY 1,2 ORDER BY 1,2
) AS A GROUP BY A.meterid;


UPDATE smas_power_consumptionpattern
SET activity_avg_weekday = (
    SELECT
        array_agg(A.AvgActivity)
    FROM (
        SELECT extract(hour from readtime),
               avg(activity) as AvgActivity
        FROM smas_power_hourlyreading WHERE  smas_power_hourlyreading.meterid=smas_power_consumptionpattern.meterid AND
                                      smas_power_hourlyreading.holidayFlag=0
...
SET activity_avg_holiday = (
    ...
    FROM (
        SELECT extract(hour from readtime),
               avg(activity) as AvgActivity
        FROM smas_power_hourlyreading WHERE  smas_power_hourlyreading.meterid=smas_power_consumptionpattern.meterid AND
                                      smas_power_hourlyreading.holidayFlag=1
        GROUP BY 1 ORDER BY 1
    ) A
);
...
DROP TABLE smas_power_norm_consumptionpattern;
CREATE TABLE smas_power_norm_consumptionpattern(
    meterid integer primary key,
    tenpct float8[],
    tenpct_C integer,
    fiftypct float8[],
...
DROP TABLE smas_power_norm_consumptionpattern_centroid;
CREATE TABLE smas_power_norm_consumptionpattern_centroid(
    measure varchar(30),
    clusterid integer,
    cent float8[]
);
...
INSERT INTO smas_power_norm_consumptionpattern
(
	meterid,
	tenpct,
	fiftypct,
...
FROM (SELECT meterid,
	...
	FROM smas_power_consumptionpattern) A,
(
SELECT meterid,
	sum(e10) as t10,
	sum(e50) as t50,
...
FROM (
	SELECT meterid,
		...
		unnest(activity_avg_weekday) as e_avg_w,
		unnest(activity_avg_holiday) as e_avg_h
	FROM smas_power_consumptionpattern) C
GROUP BY meterid
HAVING sum(e10)>0 AND
       sum(e50)>0 AND
       sum(e90)>0 AND
       sum(e_avg)>0 AND
       sum(e_avg_w)>0 AND
...
CREATE OR REPLACE FUNCTION smas_power_kcs(k integer, tablename character varying, measure character varying)
  RETURNS text AS
$BODY$
    #import sys,logging
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION smas_power_kcs(integer, character varying, character varying)
  OWNER TO xiliu;

CREATE OR REPLACE FUNCTION smas_power_run_clustering(k integer)
  RETURNS text AS
$BODY$
    tables = ['smas_power_consumptionpattern', 'smas_power_norm_consumptionpattern']
    measures = ['tenpct', 'fiftypct','ninetypct','average', 'activity_avg_weekday', 'activity_avg_holiday']
    for tbl in tables:
        for measure in measures:
            plpy.execute("SELECT smas_power_KCS(%d, '%s', '%s')" % (k, tbl, measure))
$BODY$
  LANGUAGE plpythonu VOLATILE
  COST 100;
ALTER FUNCTION smas_power_run_clustering(integer)
  OWNER TO xiliu;
